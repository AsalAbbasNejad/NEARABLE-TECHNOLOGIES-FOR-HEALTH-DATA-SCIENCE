# ============================================================
# rppg_core.pipeline
# Importable, headless rPPG / HR / RR pipeline.
#
# Derived from final_with_PR_n_metrics.py. The signal-processing,
# HR/RR estimation, motion-classification and evaluation code is
# unchanged; only the CLI/Colab/display layer and the top-level
# execution block were refactored into the analyze() function at
...
# so the pipeline can render plots to PNG files on a headless server.
...
# ------------------------------------------------------------
# Headless display / output layer.
# Replaces the original CLI + Colab + matplotlib-window logic.
# Plots are always saved as PNG files into the active output
# directory; interactive previews are disabled.
# ------------------------------------------------------------

# No interactive previews when running as a library / backend.
SHOW_RPPG_PREVIEW = False
SHOW_HEAD_TRACKING_PREVIEW = False

# Active output directory + plot counter. analyze() points these at a
# per-run folder and resets the counter before each run.
PLOT_OUTPUT_DIR = Path("rppg_plot_outputs")
_plot_counter = 0


def set_output_dir(out_dir):
    """Point plot output at out_dir and reset the plot counter."""
    global PLOT_OUTPUT_DIR, _plot_counter
    PLOT_OUTPUT_DIR = Path(out_dir)
    PLOT_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    _plot_counter = 0


def cv2_imshow(frame):
    # Headless: previews are disabled.
    return None
...
def show_plot(filename=None):
    ...
    PLOT_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    _plot_counter += 1
    output_name = filename or f"plot_{_plot_counter:02d}.png"
    plt.savefig(PLOT_OUTPUT_DIR / output_name, dpi=150, bbox_inches="tight")
    plt.close()


def infer_csv_path_from_video(video_path):
    path = Path(video_path)
    csv_name = path.name.replace("_rppg_input_", "_cms50d_sync_data_")
    csv_path = path.with_name(Path(csv_name).with_suffix(".csv").name)
...
def _build_summary(rppg_results, predicted_class, confidence, out_dir):
    keys = [
        "consensus_hr", "hr_green_fft", "hr_pos_fft", "hr_green_peak",
        "hr_pos_peak", "peak_guidance_hr", "rr_final", "rr_welch_bpm",
        "rr_hilbert_bpm", "green_reliability", "pos_reliability",
        "green_window_mad", "pos_window_mad", "sampling_rate", "duration",
        "selected_pipeline", "motion_peak_corrected", "left_right_corrected",
        "left_right_low_lock_corrected", "left_right_high_hr_corrected",
    ]
    out = {k: _json_safe(rppg_results.get(k)) for k in keys}
    out["motion_class"] = predicted_class
    out["motion_confidence"] = _json_safe(confidence)
    out["plots"] = sorted(p.name for p in Path(out_dir).glob("*.png"))
    return out
    ...
    def write(self, data):
        for s in self._streams:
            try:
                ...
                # some characters we print (≈, ±, ✓, →). Degrade gracefully on
                # that stream instead of aborting the whole run.
...
def analyze(video_path, csv_path=None, out_dir="rppg_plot_outputs"):
    """Run the offline pipeline and capture its console output to log.txt.

    Thin wrapper around _run_pipeline that tees stdout to ``out_dir/log.txt``
    so the backend/UI can surface the run log. Returns the summary dict with
    an added ``log`` key naming the log file.
    """
    import contextlib
    import sys
    ...
    with open(log_file, "w", encoding="utf-8") as fh:
        ...
        try:
            with contextlib.redirect_stdout(tee):
                summary = _run_pipeline(video_path, csv_path=csv_path, out_dir=out_dir)
        finally:
            fh.flush()

    summary["log"] = log_file.name
    return summary


def _run_pipeline(video_path, csv_path=None, out_dir="rppg_plot_outputs"):
    """Run the full offline rPPG pipeline on a recorded video.

    Parameters
    ----------
    video_path : str | Path
        Path to the input face video (.avi/.mp4).
    csv_path : str | Path | None
        Optional paired CMS50D CSV; when given, metric evaluation runs.
    out_dir : str | Path
        Directory where plot PNGs are written.

    Returns a JSON-serialisable summary dict (HR/RR/motion/reliability
    scalars plus the list of generated plot filenames).
    """
    ...
    if CSV_path is None:
        ...
        # --- Pulse Amplitude Variability plot ---
        fig = plt.figure(figsize=(9, 5))
        ax  = fig.add_subplot(1, 1, 1)
        if HAS_WAVEFORM and len(peaks_ppg) >= 4:
            ax.plot(peaks_ppg / fs_ppg, wf_det[peaks_ppg], "o-",
                    color="#00838F", markersize=4, linewidth=1)
            ax.set_title(f"Pulse Amplitude Variability  CV={pav_cv:.1f}%")
            ax.set_xlabel("Time (s)")
            ax.set_ylabel("Peak amplitude")
        else:
            ax.text(0.5, 0.5, "No waveform data", ha="center", va="center",
                    transform=ax.transAxes, fontsize=10, color="gray")
            ax.set_title("Pulse Amplitude Variability")
        ax.grid(alpha=0.3)

        plt.savefig(str(PLOT_OUTPUT_DIR / "rppg_evaluation_dashboard.png"), dpi=150, bbox_inches="tight")
        show_plot("rppg_evaluation_dashboard.png")
    ...
    return _build_summary(rppg_results, predicted_class, confidence, PLOT_OUTPUT_DIR)
