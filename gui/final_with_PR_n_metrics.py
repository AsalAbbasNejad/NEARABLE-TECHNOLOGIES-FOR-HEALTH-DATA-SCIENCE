...
try:
    from google.colab.patches import cv2_imshow as _colab_cv2_imshow
    from google.colab import files as _colab_files
    IN_COLAB = True
except ImportError:
    _colab_cv2_imshow = None
    _colab_files = None
    IN_COLAB = False
...
def parse_cli_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the offline rPPG pipeline on a local video file."
    )
    parser.add_argument(
        "video_path",
        nargs="?",
        help="Path to the input video. Defaults to the first reza/*_rppg_input_*.avi sample.",
    )
    parser.add_argument(
        "--csv",
        dest="csv_path",
        help="Optional CMS50D CSV path for metric evaluation. Defaults to the paired sample CSV.",
    )
    parser.add_argument(
        "--skip-eval",
        action="store_true",
        help="Run rPPG/RR estimation but skip the CMS50D metric-evaluation section.",
    )
    parser.add_argument(
        "--show-previews",
        action="store_true",
        help="Display OpenCV preview images when running locally.",
    )
    parser.add_argument(
        "--show-plots",
        action="store_true",
        help="Display matplotlib windows when running locally. Otherwise plots are saved under rppg_plot_outputs/.",
    )
...
if not IN_COLAB and not CLI_ARGS.show_previews:
    SHOW_RPPG_PREVIEW = False


def cv2_imshow(frame):
    if IN_COLAB:
        _colab_cv2_imshow(frame)
    elif CLI_ARGS.show_previews:
...
def show_plot(filename=None):
    ...
    if IN_COLAB or CLI_ARGS.show_plots:
        plt.show()
        return
...
def resolve_video_path():
    ...
    sample_videos = sorted(Path("reza").glob("*_rppg_input_*.avi"))

    if len(sample_videos) == 0:
        raise RuntimeError(
            "No video path provided and no sample videos found under reza/*_rppg_input_*.avi."
        )

    print(f"[Local] No video path provided. Using sample video: {sample_videos[0]}")
    return str(sample_videos[0])
...
def resolve_csv_path(video_path):
    if CLI_ARGS.skip_eval:
        return None

    if CLI_ARGS.csv_path:
        path = Path(CLI_ARGS.csv_path)
        if not path.exists():
    ...
    if IN_COLAB:
        ...
        uploaded_csv = _colab_files.upload()
        if not uploaded_csv:
            return None
        return list(uploaded_csv.keys())[0]

    csv_path = infer_csv_path_from_video(video_path)
    if csv_path is not None:
        print(f"[Local] Using paired CSV for metric evaluation: {csv_path}")
        return str(csv_path)
