# =========================
# Final Smart Multi-ROI + POS rPPG
# Binary Classification for:
# 1) HR Green FFT
# 2) HR POS FFT
# 3) Final Consensus HR
...
def classify_hr_binary(hr, normal_low=65, normal_high=85):
    if hr is None:
        return "Unknown", -1
...
def summarize_channel_fft(
    ...
    full_best = candidates[0]["bpm"] if len(candidates) > 0 else None
    ...
    stability_score = 0.0 if window_mad is None else 1.0 / (1.0 + window_mad)
    quality_score = float(np.median(window_qualities)) if len(window_qualities) > 0 else 0.0
...
def build_consensus_hr(green_info, pos_info):
    ...
    if green_info["window_hr"] is not None:
        values.append(green_info["window_hr"])
        weights.append(1.0 + green_info["reliability"])

    if pos_info["window_hr"] is not None:
        values.append(pos_info["window_hr"])
        weights.append(1.0 + pos_info["reliability"])

    if green_info["full_best"] is not None:
        values.append(green_info["full_best"])
        weights.append(0.45 + 0.5 * green_info["reliability"])

    if pos_info["full_best"] is not None:
        values.append(pos_info["full_best"])
...
def get_multi_rois(face, frame_shape):
    ...
    return {
        key: keep_roi_inside_frame(value, frame_shape)
...
def pos_rppg(rgb_signal, fs, window_seconds=1.6):
    ...
    for start in range(0, n - window_length + 1):
        ...
        alpha = 0.0 if std_s2 < 1e-8 else np.std(S1) / std_s2
...
# =========================
# Binary classification for each HR
# =========================

green_fft_class, green_fft_label = classify_hr_binary(
    hr_green_fft,
    NORMAL_LOW,
    NORMAL_HIGH
)

pos_fft_class, pos_fft_label = classify_hr_binary(
    hr_pos_fft,
    NORMAL_LOW,
    NORMAL_HIGH
)

consensus_class, consensus_label = classify_hr_binary(
    final_hr,
    NORMAL_LOW,
    NORMAL_HIGH
)
...
print(f"Green FFT Class: {green_fft_class}")
print(f"Green FFT Binary Label: {green_fft_label}")
...
print(f"POS FFT Class: {pos_fft_class}")
print(f"POS FFT Binary Label: {pos_fft_label}")

print("\n===== Consensus Result =====")
print(f"Final Consensus HR: {final_hr:.2f} bpm" if final_hr is not None else "Final Consensus HR: Not available")
print(f"Consensus Class: {consensus_class}")
print(f"Consensus Binary Label: {consensus_label}")

print("\n===== Binary Classification Summary =====")
print(f"Threshold: Normal if {NORMAL_LOW} <= HR <= {NORMAL_HIGH}")
print(f"HR Green FFT: {hr_green_fft:.2f} bpm -> {green_fft_class} ({green_fft_label})" if hr_green_fft is not None else "HR Green FFT: Not available")
print(f"HR POS FFT: {hr_pos_fft:.2f} bpm -> {pos_fft_class} ({pos_fft_label})" if hr_pos_fft is not None else "HR POS FFT: Not available")
print(f"Final Consensus HR: {final_hr:.2f} bpm -> {consensus_class} ({consensus_label})" if final_hr is not None else "Final Consensus HR: Not available")
...
if green_bpm_axis is not None and green_spectrum is not None:
    plt.plot(green_bpm_axis, green_spectrum, label="Green-only spectrum", alpha=0.7)

if pos_bpm_axis is not None and pos_spectrum is not None:
    plt.plot(pos_bpm_axis, pos_spectrum, label="POS spectrum", linewidth=2)
...
if final_hr is not None:
    plt.axvline(final_hr, linestyle=":", label=f"Consensus = {final_hr:.2f}")
