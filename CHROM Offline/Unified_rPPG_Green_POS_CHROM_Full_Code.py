...
def chrom_rppg(rgb_signal, fs, window_seconds=1.6):
    """
    CHROM rPPG extraction.

    This method is added for the individual extension requested by the professors.
    It uses the same RGB input and the same sliding-window logic as POS.
    Only the rPPG extraction projection is different.

    RGB channel order in this code is: R, G, B.
    """
    rgb_signal = np.asarray(rgb_signal, dtype=float)
    n = rgb_signal.shape[0]
    ...
    for start in range(0, n - window_length + 1):
        ...
        C = rgb_signal[start:end, :].T  # shape: 3 x window_length
...
def standardized_preprocess_rppg(raw_signal, fs):
    """
    Shared preprocessing used for Green, POS, and CHROM comparison.

    This is important for the extension:
    face/ROI extraction, preprocessing, HR estimation, and CMS50D reference
    must be identical for all methods. Only the rPPG extraction algorithm changes.
    """
    raw_signal = np.asarray(raw_signal, dtype=float)
    raw_signal = detrend(raw_signal)
    norm_signal = normalize_signal(raw_signal)
    ...
    return norm_signal, filt_signal


def estimate_single_method_hr(filtered_signal, fs):
    """
    Same HR estimation procedure for all three methods.
    Uses the existing windowed FFT + candidate ranking logic.
    """
    info = summarize_channel_fft(
        filtered_signal,
        fs,
        min_bpm=MIN_BPM,
        max_bpm=MAX_BPM,
    ...
    hr_fft = info["window_hr"]
    if hr_fft is None:
        hr_fft = info["full_best"]

    hr_peak = estimate_hr_peaks(
        filtered_signal,
        fs,
        min_bpm=MIN_BPM,
        max_bpm=MAX_BPM
    ...
    return hr_fft, hr_peak, info


def compute_method_metrics(estimate_hr, gt_hr):
    if estimate_hr is None or gt_hr is None:
        return {
    ...
    signed_error = float(estimate_hr - gt_hr)
    abs_error = float(abs(signed_error))
    mape = float(abs_error / (gt_hr + 1e-8) * 100.0)

    return {
        "signed_error_bpm": signed_error,
        "abs_error_bpm": abs_error,
        "mape_percent": mape,
        "within_3_bpm": bool(abs_error <= 3.0),
        "within_5_bpm": bool(abs_error <= 5.0)
    }


def run_green_pos_chrom_comparison(video_path, gt_hr=None, motion_label=None, save_plots=True):
    """
    Extension comparison required by the professors.
    ...
    same face/ROI extraction, same preprocessing, same HR estimation,
    same CMS50D reference. Only rPPG extraction changes.
    """
    print("\n" + "=" * 70)
    print("  EXTENSION: GREEN vs POS vs CHROM COMPARISON")
    print("=" * 70)
    ...
    # --- Only this rPPG extraction component changes ---
    raw_methods = {
        "Green": green_signal,
        "POS": pos_rppg(rgb_signal, fs=fs, window_seconds=POS_WINDOW_SECONDS),
        "CHROM": chrom_rppg(rgb_signal, fs=fs, window_seconds=POS_WINDOW_SECONDS)
    }

    method_outputs = {}
    rows = []

    for method_name, raw_signal in raw_methods.items():
        norm_signal, filt_signal = standardized_preprocess_rppg(raw_signal, fs)
        hr_fft, hr_peak, info = estimate_single_method_hr(filt_signal, fs)

        metrics = compute_method_metrics(hr_fft, gt_hr)
        ...
        rows.append({
            ...
            "condition": motion_label if motion_label is not None else "unknown",
            "method": method_name,
            "estimated_hr_bpm": hr_fft,
            "peak_hr_bpm": hr_peak,
            "cms50d_gt_bpm": gt_hr,
            "signed_error_bpm": metrics["signed_error_bpm"],
            "abs_error_bpm": metrics["abs_error_bpm"],
            "mape_percent": metrics["mape_percent"],
            "within_3_bpm": metrics["within_3_bpm"],
            "within_5_bpm": metrics["within_5_bpm"],
            "reliability": info["reliability"],
            "window_mad": info["window_mad"],
    ...
    if save_plots:
        ...
        for method_name in ["Green", "POS", "CHROM"]:
            plt.plot(
                relative_time,
                method_outputs[method_name]["filt"],
                label=f"{method_name} filtered",
                alpha=0.8
            )
        plt.axvline(TRIM_START_SECONDS, linestyle="--", label="FFT trim start")
        plt.xlabel("Time (s)")
        plt.ylabel("Amplitude")
        plt.title(f"{motion_label if motion_label else 'Video'}: Filtered rPPG Signals — Green vs POS vs CHROM")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig("extension_filtered_signals_green_pos_chrom.png", dpi=150, bbox_inches="tight")
        plt.show()
        ...
        for method_name in ["Green", "POS", "CHROM"]:
            ...
            if bpm_axis is not None and spectrum is not None:
                plt.plot(
                    bpm_axis,
                    spectrum,
                    label=f"{method_name} spectrum"
                )

            hr_value = method_outputs[method_name]["hr_fft"]
            if hr_value is not None:
                plt.axvline(
                    hr_value,
                    linestyle="--",
                    alpha=0.7,
                    label=f"{method_name} HR = {hr_value:.2f}"
                )
        ...
        if gt_hr is not None:
            plt.axvline(
                ...
                linestyle=":",
                linewidth=2,
                label=f"CMS50D GT = {gt_hr:.2f}"
            )

        plt.xlabel("Heart Rate (BPM)")
        plt.ylabel("FFT Magnitude")
        plt.title(f"{motion_label if motion_label else 'Video'}: FFT Comparison — Green vs POS vs CHROM")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig("extension_fft_comparison_green_pos_chrom.png", dpi=150, bbox_inches="tight")
        plt.show()

    return comparison_df, method_outputs


def summarize_extension_results(results_csv_path="extension_method_comparison_results.csv"):
    """
    Aggregates all processed videos and produces the exact tables requested:
    MAE and percentage of correct estimates within ±3 BPM and ±5 BPM
    separately for stable and zigzag conditions.
    """
    path = Path(results_csv_path)

    if not path.exists():
        print(f"[Extension] No results file found: {results_csv_path}")
        return None

    df_ext = pd.read_csv(path)

    if len(df_ext) == 0:
        print("[Extension] Results file is empty.")
        return None
    ...
    summary = (
        ...
        .agg(
            n_videos=("abs_error_bpm", "count"),
            mae_bpm=("abs_error_bpm", "mean"),
            median_ae_bpm=("abs_error_bpm", "median"),
            within_3_percent=("within_3_bpm", lambda x: 100.0 * np.mean(x.astype(bool))),
            within_5_percent=("within_5_bpm", lambda x: 100.0 * np.mean(x.astype(bool))),
    ...
    print("\n" + "=" * 70)
    print("  EXTENSION AGGREGATED RESULTS")
    print("=" * 70)
    print(summary.to_string(index=False))

    summary.to_csv("extension_summary_by_condition_method.csv", index=False)
    print("\nSaved summary → extension_summary_by_condition_method.csv")

    # Ranking by MAE within each condition
    print("\nRanking by condition (lower MAE is better):")
    for condition in summary["condition"].unique():
        sub = summary[summary["condition"] == condition].sort_values("mae_bpm")
        ranking = " > ".join(sub["method"].tolist())
        print(f"{condition}: {ranking}")
...
def plot_rppg_outputs(
    ...
    # ------------------------------------------------------------
    # CHROM time-domain signal
    # ------------------------------------------------------------
    plt.figure(figsize=(14, 5))
    plt.plot(relative_time, chrom_norm, label="CHROM normalized", alpha=0.35)
    plt.plot(relative_time, chrom_filt, label="CHROM filtered", linewidth=2)
    plt.axvline(TRIM_START_SECONDS, linestyle="--", label="FFT trim start")
    plt.xlabel("Time (s)")
    plt.ylabel("Amplitude")
    plt.title(f"{pipeline_name}: CHROM rPPG Signal")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    # ------------------------------------------------------------
    # FFT Spectrum Comparison: Green vs POS vs CHROM
    # ------------------------------------------------------------
    ...
    chrom_bpm_axis, chrom_spectrum = compute_fft_spectrum(
        chrom_filt,
        sampling_rate,
        min_bpm=MIN_BPM,
        max_bpm=MAX_BPM,
    ...
    if chrom_bpm_axis is not None and chrom_spectrum is not None:
        plt.plot(
            chrom_bpm_axis,
            chrom_spectrum,
            label="CHROM spectrum",
            linewidth=2,
            alpha=0.85
        )
    ...
    if hr_green_fft is not None:
        plt.axvline(
            ...
            linestyle="--",
            linewidth=1.5,
            label=f"Green FFT = {hr_green_fft:.2f}"
        )
    ...
    if hr_pos_fft is not None:
        plt.axvline(
            ...
            linestyle="--",
            linewidth=1.5,
            label=f"POS FFT = {hr_pos_fft:.2f}"
        )

    if hr_chrom_fft is not None:
        plt.axvline(
            hr_chrom_fft,
            linestyle="--",
            linewidth=1.5,
            label=f"CHROM FFT = {hr_chrom_fft:.2f}"
        )
    ...
    if hr_consensus is not None:
        plt.axvline(
            ...
            linestyle=":",
            linewidth=2,
            label=f"Consensus = {hr_consensus:.2f}"
        )
    ...
    if peak_guidance_hr is not None:
        plt.axvline(
            ...
            linestyle="-.",
            linewidth=1.5,
            label=f"Peak guidance = {peak_guidance_hr:.2f}"
        )

    plt.xlabel("Heart Rate (BPM)")
    plt.ylabel("FFT Magnitude")
    plt.title(f"{pipeline_name}: Smart FFT Spectrum Comparison — Green vs POS vs CHROM")
    plt.grid(True, alpha=0.35)
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(
        f"{pipeline_name.lower()}_fft_green_pos_chrom.png",
        dpi=300,
        bbox_inches="tight"
    )
    plt.show()
...
def run_full_rppg_pipeline(video_path, selected_pipeline):
    ...
    # ============================================================
    # EXTENSION: CHROM branch
    # Same RGB input, same ROI extraction, same preprocessing, same HR estimator.
    # Only the rPPG extraction projection changes.
    # ============================================================
    chrom_signal_raw = chrom_rppg(
        rgb_signal,
        fs=sampling_rate,
        window_seconds=POS_WINDOW_SECONDS
    )

    chrom_signal_raw = detrend(chrom_signal_raw)
    chrom_norm = normalize_signal(chrom_signal_raw)

    chrom_filt = bandpass_filter(
        chrom_norm,
        fs=sampling_rate,
        min_bpm=MIN_BPM,
        max_bpm=MAX_BPM
    ...
    # CHROM uses the exact same FFT estimator as Green and POS.
    chrom_info = summarize_channel_fft(
        chrom_filt,
        sampling_rate,
        min_bpm=MIN_BPM,
        max_bpm=MAX_BPM,
    ...
    hr_chrom_fft = chrom_info["window_hr"]
    if hr_chrom_fft is None:
        hr_chrom_fft = chrom_info["full_best"]
    ...
    hr_chrom_peak = estimate_hr_peaks(
        chrom_filt,
        sampling_rate,
        min_bpm=MIN_BPM,
        max_bpm=MAX_BPM
    ...
    print("\n===== Smart CHROM rPPG Results =====")
    print(f"HR CHROM FFT: {hr_chrom_fft:.2f} bpm" if hr_chrom_fft is not None else "HR CHROM FFT: Not available")
    print(f"HR CHROM Peak: {hr_chrom_peak:.2f} bpm" if hr_chrom_peak is not None else "HR CHROM Peak: Not available")
    ...
    print_candidates("CHROM FFT Debug", chrom_info)
    ...
    # Additional extension plot: filtered Green/POS/CHROM together
    relative_time = times - times[0]
    plt.figure(figsize=(14, 5))
    plt.plot(relative_time, green_filt, label=f"Green filtered | HR={hr_green_fft:.2f}" if hr_green_fft is not None else "Green filtered")
    plt.plot(relative_time, pos_filt, label=f"POS filtered | HR={hr_pos_fft:.2f}" if hr_pos_fft is not None else "POS filtered")
    plt.plot(relative_time, chrom_filt, label=f"CHROM filtered | HR={hr_chrom_fft:.2f}" if hr_chrom_fft is not None else "CHROM filtered")
    plt.axvline(TRIM_START_SECONDS, linestyle="--", label="FFT trim start")
    plt.xlabel("Time (s)")
    plt.ylabel("Amplitude")
    plt.title(f"{selected_pipeline.upper()}: Green vs POS vs CHROM filtered rPPG signals")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    "hr_pos_fft"                    : hr_pos_fft,
    "hr_pos_peak"                   : hr_pos_peak,
    "hr_chrom_fft"                  : hr_chrom_fft,
    "hr_chrom_peak"                 : hr_chrom_peak,
    "peak_guidance_hr"              : peak_guidance_hr,
...
rppg_results = {
    ...
    "hr_chrom_fft"                  : result.get("hr_chrom_fft"),
    "hr_green_peak"                 : result["hr_green_peak"],
    "hr_pos_peak"                   : result["hr_pos_peak"],
    "hr_chrom_peak"                 : result.get("hr_chrom_peak"),
    ...
    "chrom_reliability"             : result.get("chrom_reliability"),
    "green_window_mad"              : result["green_window_mad"],
    "pos_window_mad"                : result["pos_window_mad"],
    "chrom_window_mad"              : result.get("chrom_window_mad"),
...
if not uploaded_csv:
    ...
    if RPPG_CONSENSUS_HR is not None:
        ...
        # ============================================================
        # EXTENSION METRICS: compare Green, POS, CHROM against same CMS50D GT
        # ============================================================
        print("\n" + "=" * 55)
        print("  EXTENSION COMPARISON: GREEN vs POS vs CHROM")
        print("=" * 55)
        method_estimates = {
            "Green": rppg_results.get("hr_green_fft"),
            "POS": rppg_results.get("hr_pos_fft"),
            "CHROM": rppg_results.get("hr_chrom_fft"),
        }
        extension_rows = []
        for method_name, method_hr in method_estimates.items():
            if method_hr is None:
                signed_err_m = np.nan
                abs_err_m = np.nan
                within3_m = False
                within5_m = False
            else:
                signed_err_m = float(method_hr - GT_SCALAR)
                abs_err_m = float(abs(signed_err_m))
                within3_m = bool(abs_err_m <= 3.0)
                within5_m = bool(abs_err_m <= 5.0)
            extension_rows.append({
                "video": CSV_path.split("/")[-1].replace(".csv", ""),
                "condition": result["selected_pipeline"],
                "method": method_name,
                "estimated_hr_bpm": method_hr,
                "cms50d_gt_bpm": GT_SCALAR,
                "signed_error_bpm": signed_err_m,
                "abs_error_bpm": abs_err_m,
                "within_3_bpm": within3_m,
                "within_5_bpm": within5_m,
            })
        extension_df_current = pd.DataFrame(extension_rows)
        print(extension_df_current.to_string(index=False))

        EXTENSION_RESULTS_PATH = "extension_method_comparison_results.csv"
        if Path(EXTENSION_RESULTS_PATH).exists():
            old_ext = pd.read_csv(EXTENSION_RESULTS_PATH)
            extension_all = pd.concat([old_ext, extension_df_current], ignore_index=True)
            extension_all = extension_all.drop_duplicates(
                subset=["video", "condition", "method"],
                keep="last"
            )
        else:
            extension_all = extension_df_current.copy()
        extension_all.to_csv(EXTENSION_RESULTS_PATH, index=False)
        print(f"\n[Extension] Saved/updated → {EXTENSION_RESULTS_PATH}")

        extension_summary = (
            extension_all[extension_all["condition"].isin(["stable", "zigzag"])]
            .groupby(["condition", "method"], as_index=False)
            .agg(
                n_videos=("video", "nunique"),
                mae_bpm=("abs_error_bpm", "mean"),
                within_3_percent=("within_3_bpm", lambda x: 100.0 * np.mean(x)),
                within_5_percent=("within_5_bpm", lambda x: 100.0 * np.mean(x)),
            )
        )
        extension_summary.to_csv("extension_summary_by_condition_method.csv", index=False)
        print("\n[Extension] Summary by condition and method:")
        print(extension_summary.to_string(index=False))
        print("[Extension] Saved → extension_summary_by_condition_method.csv")
...
# ============================================================
# End of integrated Green/POS/CHROM extension version
# ============================================================
